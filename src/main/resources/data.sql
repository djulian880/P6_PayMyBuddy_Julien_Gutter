...
CREATE TABLE IF NOT EXISTS `PayMyBuddy`.`Connection` (
  ...
  CONSTRAINT `fk_Connection_User`
    ...
    REFERENCES `PayMyBuddy`.`User` (`id_User`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
GRANT SELECT ON TABLE `PayMyBuddy`.* TO 'paymybuddyUser';
GRANT SELECT, INSERT, TRIGGER ON TABLE `PayMyBuddy`.* TO 'paymybuddyUser';
GRANT SELECT, INSERT, TRIGGER, UPDATE, DELETE ON TABLE `PayMyBuddy`.* TO 'paymybuddyUser';
